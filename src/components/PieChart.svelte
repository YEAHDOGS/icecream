<script>
  ...
  /**
   * Platform mix of the featured / community-submitted videos in The Field.
...
{#if total === 0}
  <p class="m-0 text-[0.74rem] text-ink-2">
    No featured videos yet — the first community submissions will chart here.
  </p>
{:else}
  <div class="pie-wrap">
    <svg
      ...
      class="pie"
      role="img"
      aria-label="Featured applicant videos by platform: {slices
    ...
    >
      ...
      <text x="70" y="84" text-anchor="middle" class="pie-cap">featured</text>
    </svg>
    <ul class="m-0 p-0 list-none legend">
      {#each slices as s}
        <li>
          <span class="dot" style="background: {COLORS[s.platform]}"></span>
...
<p class="m-0 mt-2 text-[0.66rem] leading-snug text-muted">
  Featured applicants (n={total}) — community-submitted sample, not Deel's full
  applicant data.
</p>
